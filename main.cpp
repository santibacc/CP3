...
void Motores(int32_t VelM1,int32_t VelM2);
void SENSORIR();
void SigueMano();
void pulsador();
...
void decodeData(uint8_t index){
    ...
    case TESTMOTOR:
        ...
        w.u8[0]=rxData[index+1];
        w.u8[1]=rxData[index+2];
        w.u8[2]=rxData[index+3];
        w.u8[3]=rxData[index+4];
        
        VelM1=w.i32;

        w.u8[0]=rxData[index+5];
        w.u8[1]=rxData[index+6];
        w.u8[2]=rxData[index+7];
        w.u8[3]=rxData[index+8];
        VelM2=w.i32;
...
void Motores(int32_t VelM1,int32_t VelM2){ //sent =1 AVANZA HACIA DELANTE 0 hacia atras
    ...
    M2.pulsewidth_us(abs(VelM2)*250); //motor derecho tiene mas fuerza que el izquierdo
}
